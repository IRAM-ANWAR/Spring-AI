...
public class AudioController {
	...
	public String audioTranscription() {
		...
		OpenAiAudioTranscriptionOptions options = OpenAiAudioTranscriptionOptions.builder().language("es")
		        .responseFormat(OpenAiAudioApi.TranscriptResponseFormat.SRT).temperature(0.5f).build();
		AudioTranscriptionPrompt prompt = new AudioTranscriptionPrompt(new ClassPathResource("audio/sample-audio.m4a"),
		        options);
		return this.openAiAudioTranscriptionModel.call(prompt).getResult().getOutput();
	}
	...
	public ResponseEntity<Resource> generateAudio(@PathVariable("prompt") String prompt) {
		...
		OpenAiAudioSpeechOptions options = OpenAiAudioSpeechOptions.builder()
		        ...
		        .voice(OpenAiAudioApi.SpeechRequest.Voice.NOVA).speed(0.5f).build();
		...
		return ResponseEntity.ok().contentType(MediaType.APPLICATION_OCTET_STREAM)
		        ...
		        .header(HttpHeaders.CONTENT_DISPOSITION,
		                ContentDisposition.attachment().filename("whatever.mp3").build().toString())
